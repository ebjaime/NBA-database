...
def printScores():
    ...
    url_scores="http://www.basketball-reference.com/boxscores/"
    r_scores=requests.get(url_scores)
    soup_scores=BeautifulSoup(r_scores.text, "lxml")

    scores=[]

    div=soup_scores.findAll("div", {"class":"game_summaries"})
    tables=div[0].findAll("table", {"class":"teams"})
    tables_stats=div[0].findAll("table", {"class":"stats"})

    for t in tables:
        s=[]
        table_rows=t.select("tbody")[0].findAll("tr")
        t1=table_rows[0].findAll("td")[0].text.strip()
        t2=table_rows[1].findAll("td")[0].text.strip()
        t1_p=table_rows[0].findAll("td")[1].text.strip()
        t2_p=table_rows[1].findAll("td")[1].text.strip()

        t1_s=[t1, t1_p]
        t2_s=[t2, t2_p]
        scores.append([t1_s, t2_s])

    stats=[]
    for t in tables_stats:
        table_rows_stats=t.select("tbody")[0].findAll("tr")
        most_pp=table_rows_stats[0].findAll("td")[1].text.strip()
        most_rp=table_rows_stats[1].findAll("td")[1].text.strip()
        most_p=table_rows_stats[0].findAll("td")[2].text.strip()
        most_r=table_rows_stats[1].findAll("td")[2].text.strip()

        points=[most_pp, most_p]
        rebounds=[most_rp, most_r]

        stats.append([points, rebounds])

    for i in range(len(scores)):

        print(scores[i][0][0].upper() + " "+scores[i][0][1]+" - "+ scores[i][1][1]+" "+scores[i][1][0].upper() )
        print stats[i][0][0], ",",stats[i][0][1], "points"
        print stats[i][1][0], ",", stats[i][1][1], "rebounds\n"
        # print "\n"
